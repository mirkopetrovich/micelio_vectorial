...
void ofApp::draw(){
    ...
    for (int i=0;i<micelio.size();i++) {                    // loop por todas las hifas
        ...
        if (ofRandom(0,30)<1) micelio[i].velocidad.x=mouseX-micelio[i].posicion.x;
       
     

       
    
       micelio[i].velocidad*=80;
        micelio[i].velocidad.normalize();

        
        
        if (micelio[i].puntos.size()>5) micelio[i].puntos.erase(micelio[i].puntos.begin());
